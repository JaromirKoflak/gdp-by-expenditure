...
get_unsd_data = function() {
  ...
  gdp_constant <- read_excel(tf1, skip = 2) %>%
    pivot_longer(!c(1:3), names_to = "Year", values_to = "constant") %>% 
    rename(CountryID = `Area/CountryID`)
  
  url2 <- "https://unstats.un.org/unsd/amaapi/api/file/4"
...
remove_regions = function(df){
  url3 <- "https://unstats.un.org/unsd/amaapi/api/file/7"
  GET(url3, write_disk(tf3 <- tempfile(fileext = ".xlsx")))
  region_codes <- read_excel(tf3, skip = 2) %>% 
...
convert_to_millions = function(df) {
  df %>% 
    mutate(
...
calculate_industry_and_services = function(df) {
  industry = df %>% 
    filter(
      IndicatorName %in% c("Mining, manufacturing, utilities", "Construction")
    ) %>% 
  ...
  services = df %>% 
    filter(
      IndicatorName %in% c("Wholesale, retail trade, restaurants and hotels", "Transport, storage and communications", "Other activities")
    ) %>% 
    group_by(Economy_Code, Economy_Label, Year, Prices) %>% 
    summarise(Value = sum(Value)) %>% 
